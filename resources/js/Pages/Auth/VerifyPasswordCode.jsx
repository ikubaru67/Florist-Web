...
export default function VerifyPasswordCode({ email, status }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('password.verify.code.submit'), {
            onError: () => reset('code'),
        });
    };
    ...
    const resendCode = (e) => {
        e.preventDefault();
        post(route('password.resend.code'), {
            data: { email: email },
        });
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-pink-100 via-purple-50 to-blue-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
            <Head title="Verifikasi Kode - Florist" />
